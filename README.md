...
- **术语/规则保护**：可配置受保护术语、自定义翻译规则或导入术语 CSV，降低品牌名、型号和技术缩写被误译的风险。
...
在「批量翻译」页的「翻译规则（可选）」区域，可以为本次批量任务配置术语保护和自定义翻译规则。适用场景包括：

- **品牌名/商标**：例如 `Digital Power`、公司名、产品线名称。
- **产品型号**：例如 `SUN2000-10KTL-M1`、SKU、设备型号。
- **技术缩写**：例如 `ESS`、`PV`、`MPPT`、行业标准缩写。
- **固定写法**：例如带大小写、空格、连字符或数字组合的专有名称。
...
也可以点击「上传术语 CSV」批量导入术语表。CSV 会优先读取 `term`、`protected_term`、`术语`、`词条` 等列名；如果没有匹配列名，则读取第一列非空值并追加到受保护术语列表。

### 工作方式与限制

- 工具会把术语和规则格式化为 prompt 片段，并用 `<PROTECTED_TERMS>` 包裹术语清单、用 `<CUSTOM_RULES>` 包裹自定义规则。
- 规则会注入初译、专家建议和改译三个阶段：初译阶段从一开始保护术语；专家建议阶段重点检查误译、漏保留或改写受保护术语；改译阶段要求最终译文严格遵守术语保护。
- 输出后会做轻量校验：如果源文包含某个受保护术语而译文缺失该术语，会在日志和结果中记录 warning。
- 由于规则通过模型 prompt 执行，无法保证 100% 强制；工具通过 prompt 约束和后处理校验降低误译风险，但不自动强行替换所有术语，以避免破坏目标语言语序和标点。

## 批量翻译流程

1. 在「上传 CSV」区域点击「上传CSV文件」，或将 CSV 拖入上传区域。
2. 如需复用标准译文，在「标准翻译库（可选）」上传标准翻译 CSV，并选择命中后“直接使用”或“仍调用完整 DSL 评审”。
3. 如需保护品牌名、产品型号、技术缩写或商标，在「翻译规则（可选）」填写受保护术语、自定义规则，或上传术语 CSV。
4. 在「输入列与输出方式」中选择：
   - 原文列
   - 源语言
   - 国家/地区（可选）
