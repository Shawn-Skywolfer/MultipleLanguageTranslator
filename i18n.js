(() => {
  ...
  const english = {
    ...
    '模型能力': 'Model Capability', '自动识别': 'Auto-detect', '多模态（文字 + 图片）': 'Multimodal (Text + Images)', '仅文本': 'Text Only',
    ...
    '使用多模态模型翻译 PPT 图片中的文字': 'Translate Text in PPT Images with a Multimodal Model',
    '模型会识别图片中文字及位置，并在译文页的对应区域覆盖译文。启用后必须使用多模态模型。': 'The model detects text and its position in images, then overlays translations in the corresponding regions on translated slides. A multimodal model is required.',
    '发送前会自动压缩图片；如企业安全代理拦截图片上传，会自动切换为本地 OCR + 纯文本模型翻译。图片任务固定单并发并对 429 进行指数退避。': 'Images are compressed before upload. If a corporate security proxy blocks image uploads, the app automatically falls back to local OCR plus text-only model translation. Image tasks run serially and use exponential backoff for HTTP 429 responses.',
    '发送前会自动压缩图片；如企业安全代理拦截图片上传，会自动切换为本地 OCR + 纯文本模型翻译。图片任务固定单并发并对 429 进行指数退避。Chrome / Edge 保存后会自动重读文件并验证字节、CRC 与包结构。': 'Images are compressed before upload. If a corporate security proxy blocks image uploads, the app automatically falls back to local OCR plus text-only model translation. Image tasks run serially and use exponential backoff for HTTP 429 responses. After saving in Chrome or Edge, the app reopens the file and verifies its bytes, CRC, and package structure.',
    'PPTX 会输出“原始页保留 + 每页后新增 1 页译文页”的翻译版 PPTX。普通文本框、表格单元格和图片文字都会纳入完整性检查；任何对象翻译失败时将阻止导出，避免生成漏译文件。导出前还会校验 OOXML、关系文件和幻灯片结构。': 'The translated PPTX preserves each original slide and adds a translated slide after it. Text boxes, table cells, and image text are covered by a completeness check; export is blocked if any object fails, preventing partially translated files. OOXML, relationships, and slide structure are validated before export.',
